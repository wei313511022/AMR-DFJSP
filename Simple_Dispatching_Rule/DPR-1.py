...
RULE_NAME = "DPR-1"
RULE_TAG = "dpr-1"
DEFAULT_OUT = "dpr-1_schedule.jsonl"
DEFAULT_FIG_OUT = "dpr-1_schedule.png"

def _select_rule(jobs, machine_times, amrs, last_job, last_machine):
    max_remaining = max(job.remaining_ops() for job in jobs)
    candidates = [job for job in jobs if job.remaining_ops() == max_remaining]
