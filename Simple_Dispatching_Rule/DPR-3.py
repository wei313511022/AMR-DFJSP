#!/usr/bin/env python3
"""DPR-3 for route-aware FJSP: min average processing time."""
...
INSTANCE_INDEX = 0
OUT_PATH = "../results/dpr-3_result.jsonl"
FIG_OUT_PATH = "../results/dpr-3_result.html"
...
def _select_rule(jobs, machine_times, amrs, last_job, last_machine):
    ...
    candidates = []
    for job in jobs:
        options = job.next_options()
        avg = sum(opt.processing for opt in options) / float(len(options))
        if best is None or avg < best - _EPS:
            best = avg
            candidates = [job]
        elif abs(avg - best) <= _EPS:
